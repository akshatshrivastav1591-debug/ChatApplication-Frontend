...
export function MyProfile() {
  ...
  function toYMD(isoString) {
    if (!isoString) return "";
    return isoString.split("T")[0]; // "2005-08-24"
  }
  ...
  //Updating userProfile if profile picture is changed
  ...
  useEffect(() => {
    async function getUserProfile() {
      const response = await fetch(`${API_BASE_URL}/getMyProfile`, {
        method: "GET",
        credentials: "include",
      });
      const data = await response.json();
      if (response.ok) {
        setFetchedUserDetails(data.userProfile);
        setProfilePicture(data.userProfile.imageUrl);
      } else {
  ...
  return (
    <>
      <div className="bg-blue-100 font-bold flex flex-col items-center py-10 px-4 gap-6">
        {/* Title */}
        <h1 className="text-center text-lg">Your Profile:</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex flex-col w-full">
              ...
              <input
                ...
                ref={firstName}
                placeholder={fetchedUserDetails?.firstName ?? ""}
              />
            </div>
            ...
            <div className="flex flex-col w-full">
              ...
              <input
                className="bg-blue-200 p-2 rounded border-2 w-full"
                placeholder={fetchedUserDetails?.lastName ?? ""}
                ref={lastName}
              />
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex flex-col w-full">
              ...
              <input
                ...
                className="bg-blue-200 p-2 rounded border-2 w-full"
                ref={userDateofBirth}
                defaultValue={toYMD(fetchedUserDetails?.dateOfBirth)}
              />
            </div>
            ...
            <div className="flex flex-col w-full">
              ...
              <input
                className="bg-blue-200 p-2 rounded border-2 w-full"
                ref={userAdress}
                placeholder={fetchedUserDetails?.address ?? ""}
              />
            </div>
          </div>
        </div>
        <div className="justify-center">
          ...
          <Buttons OnChange={updateUserProfile}>Update Profile</Buttons>
        </div>
      </div>
    </>
  );
